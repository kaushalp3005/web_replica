...
export interface SendIntimationModalProps {
  ...
  onClose: () => void;
  // Optional preloads (e.g. opened from the inward page with data already entered).
  initialVehicle?: string;
  initialInvoice?: string;
  // When true, the vehicle/invoice inputs are hidden and the provided values are
  // sent as-is — the caller (inward page) owns those fields.
  transportReadOnly?: boolean;
  // Optional async hook awaited before the intimation is sent (e.g. the inward
  // persists its receiving data so the vehicle/challan are saved too).
    ...
    >
      ...
      >
        ...
        <div className="px-5 py-4 overflow-y-auto flex-1">
          ...
          {transportReadOnly ? (
            /* Vehicle + Challan come from the inward form — shown read-only here. */
            <div className="mb-3 rounded-[2px] border border-[var(--aws-border)] bg-[var(--surface-subtle)] px-3 py-2">
              <div className="text-[11px] font-bold uppercase tracking-wide text-[var(--text-muted)] mb-1">Sent with (from the inward form)</div>
              <div className="flex flex-wrap gap-x-5 gap-y-0.5 text-[12px] text-[var(--text-primary)]">
                <span>Vehicle No.: <span className="font-mono">{vehicleNo.trim() || "—"}</span></span>
                <span>Challan No.: <span className="font-mono">{invoiceNo.trim() || "—"}</span></span>
              </div>
              {(!vehicleNo.trim() || !invoiceNo.trim()) && !result ? (
                <p className="mt-1 text-[11px] text-[var(--aws-error)]">Enter Vehicle No. and Challan No. in the inward form to send.</p>
              ) : null}
            </div>
          ) : (
            <>

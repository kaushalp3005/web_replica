  ...
  const handleQRScan = useCallback(async (text: string): Promise<boolean> => {
    try {
      ...
      if (parsed && parsed.tx && parsed.bi) {
        ...
        // Only BE- needs its own endpoint (bulk-entry only). Everything else — TR- inward
        // and CR- customer-return boxes — goes to boxLookupById, which unions boxes_v2,
        // bulk_entry_boxes and rtv_boxes and resolves on the (box_id, transaction_no) pair.

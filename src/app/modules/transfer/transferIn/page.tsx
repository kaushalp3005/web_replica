          ...
          <div className="bg-white border border-[var(--aws-border)] rounded-md">
            <div className="px-4 py-3 border-b border-[var(--aws-border)] flex flex-wrap items-center justify-between gap-2">
              ...
              <div className="flex items-center gap-2">
                ...
                <button onClick={() => notWired("Generate QR ID's")} className={btn("brand", "md")}>🖨 Generate QR ID&apos;s</button>
              </div>
            </div>
                            ...
                            return (
                              <Fragment key={e.key}>
                                <tr className={st === "ok" ? "bg-emerald-50/40" : st === "issue" ? "bg-rose-50/30" : "hover:bg-gray-50/50"}>
                                  ...
                                  <td className={`${TD_CELL} max-w-[220px] truncate`}>{e.article}</td>
                                  <td className={`${TD_CELL} font-mono text-[11px] text-[var(--text-secondary)]`}>{e.synthetic ? "—" : (e.transaction_no || "—")}</td>
                                  <td className={`${TD_CELL} font-mono text-[11px]`}>{e.synthetic ? "—" : e.box_id}</td>
                                  ...
                                  <td className={`${TD_CELL} text-right tabular-nums ${iss?.net_weight ? "text-rose-700 font-medium" : ""}`}>{(iss?.net_weight ? num(iss.net_weight) : e.net_weight).toFixed(3)}</td>
                                  <td className={`${TD_CELL} text-right tabular-nums ${iss?.gross_weight ? "text-rose-700 font-medium" : ""}`}>{(iss?.gross_weight ? num(iss.gross_weight) : e.gross_weight).toFixed(3)}</td>
                                  ...
                                  <td className={`${TD_CELL} whitespace-nowrap`}>
                                    <div className="flex items-center justify-end gap-1.5">
                                      ...
                                      <button onClick={() => notWired("Print QR")} className={btn("neutral")}>🖨 Print QR</button>
                        ...
                        <div key={e.key} className={`border rounded p-2 ${st === "ok" ? "border-emerald-200 bg-emerald-50/40" : st === "issue" ? "border-rose-200 bg-rose-50/30" : "border-[var(--aws-border)]"}`}>
                          ...
                          {canAcknowledge && (
                            <div className="mt-2 flex flex-wrap gap-1.5">
                              ...
                              <button onClick={() => notWired("Print QR")} className={btn("neutral", "md")}>🖨 Print QR</button>
                            </div>
                          )}
                          {issueOpen === e.box_id && (

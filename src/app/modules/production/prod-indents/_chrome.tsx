...
// Shared header/footer chrome for the Production Indents sub-module. Mirrors
// so-creation/_chrome.tsx so the navy bar, breadcrumb, avatar button and
// footer read identically to the other Production sub-pages — only the
// breadcrumb tail ("Production Indents") and the back target differ.
...
export interface ChromeProps {
  ...
  title: string;
  /** Show the "← Back to production" button at the top of <main>. */
  showBackToProduction?: boolean;
  children: React.ReactNode;
}

export function ProdIndentsChrome({ title, showBackToProduction, children }: ChromeProps) {
      ...
      <header className="bg-[var(--aws-navy)] h-[45px] flex items-center px-6 gap-4">
        ...
        <nav className="text-[12px] text-[#d5dbdb] hidden md:flex items-center gap-2 ml-2">
          ...
          {title === "Production Indents" ? (
            <span className="text-white">Production Indents</span>
          ) : (
            <>
              <button
                onClick={() => router.push("/modules/production/prod-indents")}

...
// Shared header/footer chrome for Packing Details sub-pages. Navy bar,
// breadcrumb (Modules / Packing Details / <title>), avatar, footer — mirrors
// QcChrome / PurchaseChrome.

import { useRouter } from "next/navigation";
import { BrandMark } from "@/components/BrandMark";
import { useUserInitial } from "@/lib/user";

export interface PackingChromeProps {
  title: string;
  children: React.ReactNode;
}

export function PackingChrome({ title, children }: PackingChromeProps) {
      ...
      <header className="bg-[var(--aws-navy)] h-[45px] flex items-center px-6 gap-4">
        ...
        <nav className="text-[12px] text-[#d5dbdb] hidden md:flex items-center gap-2 ml-2">
          ...
          {title === "Packing Details" ? (
            <span className="text-white">Packing Details</span>
          ) : (
            <>
              <button onClick={() => router.push("/modules/packing-details")} className="hover:underline">Packing Details</button>
              <span>/</span>

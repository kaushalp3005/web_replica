...
// Shared header/footer chrome for Inventory Ledger pages. Mirrors the Transfer/
// Sample chrome: maroon bar, breadcrumb (Modules / Inventory Ledger / <title>),
    ...
    <div className="min-h-screen flex flex-col bg-[var(--background)]">
      <header className="bg-[var(--aws-navy)] h-[45px] flex items-center px-4 sm:px-6 gap-4">
        ...
        <nav className="text-[12px] text-[#d5dbdb] hidden md:flex items-center gap-2 ml-2">
          ...
          <span>/</span>
          {atRoot ? (
            <span className="text-white">Inventory Ledger</span>
          ) : (
            <>
              <button onClick={() => router.push("/modules/inventory-ledger")} className="hover:underline">
                Inventory Ledger
              </button>
              <span>/</span>
